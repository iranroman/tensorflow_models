...
def data_preprocess(images,labels):
	...
	idx_i = np.nonzero(np.in1d(labels[:,0],[9]))[0]
	images = images[idx_i]
	labels = labels[idx_i]
	#idx_ph4 = np.nonzero(np.in1d(labels[:,1],[3]))[0]
	#images = images[idx_ph4]
	#labels = labels[idx_ph4]
	idx_c1 = np.nonzero(np.in1d(labels[:,2],[0,1,2]))[0]
	idx_c2 = np.nonzero(np.in1d(labels[:,2],[3,4,5]))[0]
	idx_c3 = np.nonzero(np.in1d(labels[:,2],[6,7,8]))[0]
	labels[idx_c1,2] = 0
	labels[idx_c2,2] = 1
	labels[idx_c3,2] = 2
	idx = np.nonzero(np.in1d(labels[:,2],[0,1,2]))[0]
	images = images[idx]
	labels = labels[idx]
...
with tf.Session() as sess:
    ...
    for step in range(1, num_steps+1):
        batch_x, batch_y = next_batch(batch_size, X_tr, Y_tr)
