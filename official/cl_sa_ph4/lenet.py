...
Y_tr_init = np.load('../../../../experiments/tr_data/y_tr.npy')
...
Y_ts_init = np.load('../../../../experiments/ts_data/y_ts.npy')
X_tr_init = np.expand_dims(X_tr, axis=2)
X_ts_init = np.expand_dims(X_ts, axis=2)

def data_preprocess(images,labels,isubj):
	...
	labels[idx_ph3,2] = 0
	labels[idx_ph1,2] = 1
	idx_del = np.nonzero(np.in1d(labels[:,2],[3,4,5,9,10,11,12,13,14,15,16,17]))[0]
