...
def data_preprocess(images,labels):
	...
	idx_aud = np.nonzero(np.in1d(labels[:,1],[1,2,3]))[0]
	images = images[idx_aud]
	labels = labels[idx_aud]
	labels_dense = labels[:,0].astype(np.int)
...
# Network Parameters
num_classes = 21 # MNIST total classes (0-9 digits)
...
with tf.Session() as sess:
    ...
    np.save('conf_mat.npy',sess.run([conf_mat], feed_dict={X:X_ts,Y:Y_ts,keep_prob:1.0}))
